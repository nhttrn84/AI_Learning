...
class MinesweeperAI():
            ...
            for sentence2 in self.knowledge:
                if sentence1 != sentence2 and sentence1.cells and sentence2.cells:
                    if sentence1.cells.issubset(sentence2.cells):
                        ...
                        # If the new sentence provides additional information
                        if new_cells and new_count > 0:
                            frozen_cells = frozenset(new_cells)
                            if frozen_cells not in processed_cells:
                                new_sentences.append(Sentence(new_cells, new_count))
                                processed_cells.add(frozen_cells)
    ...
    def make_random_move(self):
        ...
        """
        available_moves = list(itertools.product(range(self.height), range(self.width)))
        possible_moves = [cell for cell in available_moves if cell not in self.moves_made and cell not in self.mines]
